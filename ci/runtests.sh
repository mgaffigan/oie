...
if [[ "$configuration" == "all" || "$configuration" != ubuntu-* ]]; then
    docker buildx build --load ${EXTRA_BUILDX_ARGS:-} --target jre-run -t "$ALPINE_IMAGE" .
fi
if [[ "$configuration" == "all" || "$configuration" == ubuntu-* ]]; then
    docker buildx build --load ${EXTRA_BUILDX_ARGS:-} --target jdk-run -t "$UBUNTU_IMAGE" .
fi

echo "==> Building harness image"
docker buildx build --load ${EXTRA_BUILDX_ARGS:-} --target smoketest-harness -t oie-ci-harness:local .
