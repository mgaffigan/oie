...
public class PinnedClientTrustSslContextFactoryTest {
    ...
    @Test
    public void testTrustStrategyRejectsPinnedCertificateBehindAnotherLeaf() throws Exception {
        PinnedCertificateTrustStrategy strategy = new PinnedCertificateTrustStrategy(PinnedClientTrustConfig.parse(THUMBPRINT).getPinnedThumbprints());
        // An attacker may append the (public) pinned certificate behind its own leaf.
        X509Certificate attackerLeaf = mockDifferentCertificate();
        X509Certificate pinned = mockCertificate();

        assertFalse(strategy.isTrusted(new X509Certificate[] { attackerLeaf, pinned }, "RSA"));
    }

    @Test
    public void testHostnameVerifierRejectsPinnedCertificateBehindAnotherLeaf() throws Exception {
        HostnameVerifier verifier = factory.createHostnameVerifier(PinnedClientTrustConfig.parse(THUMBPRINT), "example.com");
        X509Certificate attackerLeaf = mockDifferentCertificate();
        X509Certificate pinned = mockCertificate();
        SSLSession session = mock(SSLSession.class);
        when(session.getPeerCertificates()).thenReturn(new Certificate[] { attackerLeaf, pinned });

        assertFalse(verifier.verify("different-host", session));
    }
